...
inline void generate_case_mapping_code(const std::vector<CaseMapping> &to_upper,
                                       ...
                                       to_lower) {
  generate_mapping_inc(to_upper, "wctype_case_mapping_upper.inc");
  generate_mapping_inc(to_lower, "wctype_case_mapping_lower.inc");

  std::ofstream f("wctype_case_mapping.h");
  if (!f.is_open()) {
